...
function VisitsViewModel() {
    ...
    self.filterLabels = {
        ...
        evaluator: 'Denetleyen',
        personnel: 'Denetlenen',
        supervisor: 'Yönetici',
        status: 'Durum',
    ...
    self.buildFilterParams = function() {
        ...
        // Visits sayfası: Kullanıcı proje tipi seçmediyse, Gizli Müşteri + Fiziksel Denetim gönder
        if (projectTypes.length === 0 && projectIds.length === 0) {
            projectTypes.push('MysteryShopping');
            projectTypes.push('PhysicalAudit');
        }
    ...
    self.exportToExcel = function() {
        ...
        ApiService.downloadPost('/reports/export/excel', params, 'Ziyaretler.xlsx')
            .then(function() {
    ...
    self.exportDetailedToExcel = function() {
        ...
        ApiService.downloadPost('/reports/export/excel/detailed', params, 'Ziyaretler_Detayli.xlsx')
            .then(function() {
    ...
    self.exportCallAuditReport = function() {
        ...
        var filename = 'Ziyaret_Denetleme_Raporu_' + self.getTimestamp() + '.xlsx';
        ApiService.downloadPost('/reports/export/visit-audit', params, filename)
            .then(function() {
                toastr.success('Ziyaret Denetleme Raporu indirildi');
                self.closeReportsModal();
            })
            .catch(function(error) {
    ...
    self.exportCustomerEvaluationReport = function() {
        ...
        var filename = 'Musteri_Ziyaret_Degerlendirme_Raporu_' + self.getTimestamp() + '.xlsx';
        ApiService.downloadPost('/reports/export/visit-customer-evaluation', params, filename)
            .then(function() {
                toastr.success('Müşteri Ziyaret Değerlendirme Raporu indirildi');
                self.closeReportsModal();
            })
            .catch(function(error) {
    ...
    self.exportDetailToExcel = function() {
        ...
        var filename = 'Ziyaret_Degerlendirme_Detay_' + evaluation.evaluationId + '_' + self.getTimestamp() + '.xlsx';
        ApiService.downloadGet('/reports/evaluations/' + evaluation.evaluationId + '/visit-export', filename)
            .then(function() {
...
$(document).ready(function() {
    if (document.getElementById('visits-app')) {
        ko.applyBindings(new VisitsViewModel(), document.getElementById('visits-app'));
    }
});
